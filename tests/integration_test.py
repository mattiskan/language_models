...
def example_corpus():
    corpus = Corpus(2)
    corpus.add_sentence('John read Moby Dick'.split())
    corpus.add_sentence('Mary read a different book'.split())
    corpus.add_sentence('She read a book by Cher'.split())
...
def test_integration_no_smoothing(example_corpus):
    ...
    estimated_probability = sentence_prob('John read a book'.split(), model, example_corpus)
    assert approx(estimated_probability, 0.06, delta=0.01)
    assert sentence_prob('Cher read a book'.split(), model, example_corpus) == 0
...
def test_integration_additive_smoothing(example_corpus):
    ...
    estimate = sentence_prob('John read a book'.split(), model, example_corpus)
